...
def train(S,A,returns, old_log_probs):
    ...
    for i in range(POLICY_TRAIN_ITERS):
        ...
        delta = returns - V(S).view(-1)
        log_probs = torch.nn.LogSoftmax(dim=-1)(pi(S)).gather(1, A.view(-1, 1)).view(-1)
        n = torch.arange(S.size(0)).to(DEVICE)
        
        objective1 = -(delta * V(S).view(-1)).sum()
        log_ratio = log_probs - old_log_probs
        ratio = torch.exp(log_ratio)
        ag_approx = ratio * delta
        ag_clip = torch.clamp(ratio, torch.tensor(1 - CLIP_PARAM).to(DEVICE), torch.tensor(1 + CLIP_PARAM).to(DEVICE)) * delta
        objective2 = -torch.min(ag_approx, ag_clip).sum()
